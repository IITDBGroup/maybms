...
%type <node>	stmt schema_stmt
		...
		RemoveFuncStmt RemoveOperStmt RenameStmt RepairKeyFrom RepairKeyStmt RevokeStmt RevokeRoleStmt
...
%type <node> PickTuplesStmt
%type <chr> OptPickingType
%type <target> OptProb
%type <target> repairkey_target_el
%type <list> repairkey_target_list
...
%type <target>	target_el single_set_clause set_target insert_column_item WEIGHTBY
...
%type <boolean> opt_restrict opt_asymmetric
/*
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	PARSER PARTIAL PASSWORD PLACING PLANS POSITION POSSIBLE /* MAYBMS */
	PRECISION PRESERVE PREPARE PREPARED PRIMARY
	PRIOR PRIVILEGES PROBABILITY /* MAYBMS */ PROCEDURAL PROCEDURE

	QUOTE

	READ REAL REASSIGN RECHECK REFERENCES REINDEX RELATIVE_P RELEASE RENAME REPAIR
	REPEATABLE REPLACE REPLICA RESET RESTART RESTRICT RETURNING RETURNS REVOKE PICK /* MAYBMS */
	...
	TRUNCATE TRUSTED TUPLES /* MAYBMS */ TYPE_P
	...
	WEIGHT WHEN WHERE WHITESPACE_P WITH WITHOUT WORK WRITE
		...
		CREATE OptTemp TABLE create_as_target AS SelectStmt
				{
					...
					$4->rel->istemp = $2;
					n->intoClause = $4;
					n->intoClause->tabletype = TABLETYPE_CERTAIN;
					$$ = $6;
				}
...
/*
 * Syntax for creating tuple-independent relations: 
 * pick tuples from <certain-query> [independently] [with probability <attribute>];
 * “independently” is optional and the default (further alternatives could be 
 * added in the future)
 * “with probability …” is optional; if left out, tuples are picked with 
 * probability 0.5.
 */

PickTuplesStmt:
	PICK TUPLES FROM RepairKeyFrom OptPickingType OptProb
	{
		SelectStmt *n = (SelectStmt *)$4;
		
		if (n->pickingType == 'I')
			elog(ERROR, "Query not supported:pick tuples not supported on uncertain "\
				"relations");
...
/*
 * Repari-key statement is in fact a SelectStmt.
 *
 */
 
RepairKeyStmt:
		REPAIR KEY repairkey_target_list IN_P RepairKeyFrom WEIGHTBY
				{					
					SelectStmt *n = (SelectStmt *)$5;
					
					if (n->repairkey != NULL)
						elog(ERROR, "Query not supported:repair key not supported on uncertain "\
							"relations");
...
/* IMPORTANT NOTE: When we removed the parentheses around the target list in 
 * repair-key, we got 1 shift/reduce conflict. This conflicts with the rules:
 *
 *  a_expr: a_expr IN_P in_expr // IN_P is token "in"
 *         | other rules ....
 *
 * in_expr:	select_with_parens   // select with parentheses
 *		    | other rules ....
 *		
 * target_list:
 *			target_el								
 ...
 *           | other rules...
 *           
 *   Because ( a_expr IN_P in_expr ) is a_expr, so the select in repair-key 
 *   is absorbed into an expression, which is a target list element. 
 *   The solution is to create a new terminal repairkey_target_list in which 
 *   a target element has an higher precedence than IN_P and use this instead of 
 *   the the original target list in Postgres.  
 */
repairkey_target_list:
			repairkey_target_el				                        
				{ $$ = list_make1($1); }
			| repairkey_target_list ',' repairkey_target_el				
			 	{ $$ = lappend($1, $3); }
		;

repairkey_target_el:	a_expr   %prec UMINUS
				{
					$$ = makeNode(ResTarget);
					$$->name = NULL;
					$$->indirection = NIL;
 ...
 */
simple_select:
			SELECT OptPossible opt_distinct target_list
...
opt_asymmetric: ASYMMETRIC			{ $$ = TRUE; }
			| /*EMPTY*/				{ $$ = FALSE; }
		;
